...
layout(location = LOCATION_START+1) in float light0;
layout(location = LOCATION_START+2) in float light1;
layout(location = LOCATION_START+3) in float light2;
layout(location = LOCATION_START+4) in float light3;
...
layout(location = 19) out flat int lightCount;
layout(location = 20) out flat int lightIndexes[NR_LIGHTS];
...
void main() {
	...
	int indexes[NR_LIGHTS] = { int(light0), int(light1), int(light2), int(light3) };
