...
const modFileData = `module game

go %s
`

const gameFileData = `package main
...
func (pfs *FileSystem) createCodeProject() error {
	...
	workFile := []byte(fmt.Sprintf(workFileData, GoVersion))
	if err := pfs.WriteFile(ProjectWorkFile, workFile, os.ModePerm); err != nil {
		return err
	}
	modFile := []byte(fmt.Sprintf(modFileData, GoVersion))
	if err := pfs.WriteFile(ProjectModFile, modFile, os.ModePerm); err != nil {
		return err
	}
	if err := pfs.WriteFile(ProjectLaunchJsonFile, []byte(launchJsonFileData), os.ModePerm); err != nil {
		return err
	}
	if err := pfs.WriteFile(ProjectCodeGame, []byte(gameFileData), os.ModePerm); err != nil {
		return err
	}
