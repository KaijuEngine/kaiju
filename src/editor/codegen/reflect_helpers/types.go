...
// CanonicalTypeName maps the runtime names of the generic matrix vector and
// color implementations back to the public aliases used in entity-data source.
// reflect reports matrix.Vec3 as "Vec3T[float32]", for example, because Vec3
// is an alias, and matrix.Color as "ColorT[float32]". The editor's field
// classification and default parser should continue to treat those types as
// "Vec3" and "Color".
func CanonicalTypeName(typeName string) string {
	for _, vectorName := range []string{"Color", "Vec2", "Vec3", "Vec4"} {
