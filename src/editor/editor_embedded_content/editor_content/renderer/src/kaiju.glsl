...
// Tessellation helper functions for TCS/TES shaders.
// These avoid duplication across terrain.tesc / terrain.tese and future shaders.
// Compatible with kaiju.glsl style (#version 460, existing UBOs like cameraPosition,
// worldPosition() helper assumed available via includes).
float calcTessFactor(vec3 worldPos, vec3 camPos, float minTess, float maxTess, float distNear, float distFar) {
    ...
    return mix(minTess, maxTess, factor * factor);  // quadratic falloff for smoother transition
}
...
vec4 interpolate4(vec4 a, vec4 b, vec4 c, vec3 bary) {
    return a * bary.x + b * bary.y + c * bary.z;
}

float interpolate1(float a, float b, float c, vec3 bary) {
    return a * bary.x + b * bary.y + c * bary.z;
}

// For normals (re-normalize after interpolation for smoothness in TES)
vec3 interpolateNormal(vec3 a, vec3 b, vec3 c, vec3 bary) {
    vec3 n = interpolate3(a, b, c, bary);
    return normalize(n);
}

// Helper for common terrain attributes (colors, tints often vec4)
vec4 interpolateColor(vec4 a, vec4 b, vec4 c, vec3 bary) {
    return interpolate4(a, b, c, bary);
}
