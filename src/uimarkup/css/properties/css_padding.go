...
func paddingSizeFromString(elm markup.DocElement, str string, idx matrix.VectorComponent, window *windowing.Window) (matrix.Vec4, error) {
...
func (p Padding) Process(panel *ui.Panel, elm markup.DocElement, values []rules.PropertyValue, host *engine.Host) error {
	var err error
	if len(values) == 1 {
		// all
