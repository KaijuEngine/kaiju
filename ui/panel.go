...
func NewPanel(host *engine.Host, anchor Anchor) *Panel {
	panel := &Panel{
		scrollEvent:       -1,
		scrollSpeed:       30.0,
...
func (panel *Panel) onScroll() {
	...
	if (panel.scrollDirection & PanelScrollDirectionHorizontal) != 0 {
		x := max(0.0, min(panel.maxScroll.X(), delta.X()+panel.offset.X()))
		panel.scroll.SetX(x)
	}
	if (panel.scrollDirection & PanelScrollDirectionVertical) != 0 {
		y := max(0.0, min(panel.maxScroll.Y(), delta.Y()+panel.offset.Y()))
		panel.scroll.SetY(y)
	}
	panel.SetDirty(DirtyTypeLayout)
...
func (panel *Panel) recreateDrawing() {
	...
	panel.shaderData.CancelDestroy()
}
...
func (panel *Panel) ensureBGExists(tex *rendering.Texture) {
	if !panel.drawing.IsValid() {
		...
		panel.shaderData.Scissor = panel.scissor
